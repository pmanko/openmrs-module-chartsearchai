...
- Do not declare a problem "architecturally impossible" after exhausting only threshold-based approaches. Combine mechanisms: scoping (when a gate fires), thresholds (how strict it is), and baseline verification (are "regressions" actually improvements). Each alone may fail where the combination succeeds.
- Each embedding model has its own tuning in `PipelineConfig.forModel()`. When a pipeline change works for one model but regresses another, make it model-specific via a `PipelineConfig` parameter — do not use a one-size-fits-all constant that penalizes the better model. Run both model test suites independently: `EnrichedRetrievalEvalTest` with default (L6-v2) AND with `-Dchartsearchai.eval.model=medcpt`.
