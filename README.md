...
**Prompt-stability caveat — when full-chart mode is actually faster on small charts.** Querystore selects a different top-K record set for each question, so the prompt body changes between consecutive queries on the same patient — breaking the KV-cache reuse the [latency table above](#2-download-the-llm-model-local-mode-only) relies on. On large charts this is expected to be the right trade (a small top-K prompt is cheaper to prefill from scratch than the whole chart). On *small* charts that fit comfortably in the LLM context, the opposite is true: with `chartsearchai.querystore.enabled=false` and `chartsearchai.embedding.preFilter=false`, `ChartBuildingStrategy` returns the same cached full chart for every question on a patient, so the `<system> + <chart>` prefix stays stable across calls and the KV cache reuses it. The payoff is contingent on the [Warmup](#warmup) endpoint priming that prefix before the user's first question — without it, the first question still pays the full cold-prefill cost. On the demo (CPU-only Gemma 4 E2B, Betty's ~1.8K-token chart, warmup primed): with querystore on, the measured first ask is ~11 s and each different follow-up should stay in roughly the same range; with querystore off, the first ask is ~10 s and follow-ups drop to ~4–7 s (measured across three different questions in sequence).
