...
- **Apple Silicon vs Intel Mac:** Apple Silicon is the supported Mac target and runs the bundled database out of the box. Intel (x86_64) Macs have no bundled MariaDB (no maintained x86_64 macOS build exists), so install one with `brew install mariadb` and the standalone uses it automatically — but this only applies on macOS 14+ Intel hardware; otherwise prefer Apple Silicon, Windows, or Docker.
