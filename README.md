...
| `chartsearchai.drugSafety.warnOnInteractions` | `true` | Flag a drug named in the answer that interacts with one of the patient's active orders — by dataset rule, shared ATC subgroup (duplicate therapy), or shared curated cross-reactivity group (additive / duplicate-class therapy across branches). Most specific match wins |
| `chartsearchai.drugSafety.warnOnContraindications` | `true` | Flag a drug named in the answer that is contraindicated by an active allergy or condition — by dataset rule, same-drug/same-ATC-subgroup allergy, or an allergy in the same curated cross-reactivity group (cross-branch). Most specific match wins |
