 ...
 * <pre>
 *   {
 *     "reasoning": string,      // scratchpad emitted first; parsed but not surfaced
 *     "answer": string,         // prose, with [N] citation markers
 *     "citations": int[],       // referenced record indices
 *     "blocks": [               // additive structured tables for tabular answers
 *       {
 *         "kind": "table",
 *         "title": string,
 ...
 * format is compiled to a grammar that emits object properties in schema order, so making
 * {@code reasoning} first forces the model to think before it commits to an answer. The reasoning
 * field is the model's scratchpad: {@link LlmAnswerExtractor} reads only {@code answer},
 * {@code citations}, and {@code blocks} for clinician-facing output.
 *
 * <p>The {@code blocks} field tells the model to emit structured tables when the answer is naturally
 * tabular (medication lists, lab values, allergies, vitals). Single-fact answers leave
 * {@code blocks} empty. Per structured-output rules, each nested object carries
 * {@code additionalProperties: false} and a {@code required} list where the grammar backend needs
