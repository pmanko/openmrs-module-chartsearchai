...
Key design choices:
- Serializers produce concise text without dates. Dates are omitted for three reasons:
  ...
  2. **Dates are in the API response**: The actual date is included in the API response's `references` array for the UI to display alongside each citation.
  ...
  **Exception**: `PatientProgramTextSerializer` includes enrollment and completion dates because both are intrinsic clinical facts about the enrollment — the LLM has no other way to see them, since citation labels only carry the record's position, not the actual date.
