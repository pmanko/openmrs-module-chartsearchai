...
- `OrderTextSerializer` handles both base orders and `DrugOrder` subclass. Base orders include concept name, action, urgency, instructions, reason, and date stopped. Drug orders additionally include drug name (coded or non-coded), dose/units, route, frequency, duration/units, as-needed flag with condition, and dosing instructions. Omitted: orderer (who placed the order — administrative), care setting (inpatient/outpatient — contextual metadata), fulfiller status/comments (pharmacy workflow, not prescription content), quantity/refills/brand name/dispense-as-written (dispensing logistics now covered by `MedicationDispenseTextSerializer`).
