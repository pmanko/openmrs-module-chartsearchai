...
**First-sentence embedding.** Only the first sentence of each serialized record (up to the first `. `) is used for embedding computation. The first sentence captures the record type and primary concept (e.g., `"Allergy: Beef (food allergen)"` from a longer text that continues with severity and reactions). This focuses the embedding vector on the core clinical meaning rather than diluting it with secondary details like dosing instructions, action codes, or urgency flags that have low semantic relevance to clinical queries. The full serialized text is still stored in `textContent` for debugging and is sent to the LLM at query time.

**Semantic embedding prefixes.** Each record's first sentence is additionally prepended with a type-specific prefix before computing embeddings (but not in the LLM prompt). For example, a drug order is embedded as `"Medication prescription: Drug order: Azithromycin..."` while an allergy is embedded as `"Patient allergy: Allergy: Penicillin..."`. This shifts the embedding vectors toward the right semantic space, so medication queries rank drug orders higher. Prefixes are further specialized by order sub-type: `"Medication prescription:"` for drug orders, `"Lab or diagnostic test:"` for test orders, and `"Clinical referral:"` for referral orders. In testing, this moved drug orders from 8th/9th place to 1st/3rd place for medication queries.
